...
func Test_TypedValueInt(t *testing.T) {
	...
	assert.Equal(t, len(tv.Data), 8)
	assert.DeepEqual(t, tv.Data, []uint8{0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x80})

	testConversion(t, (*TypedValue)(tv))

	tv = NewInt64(12345678)
	assert.Equal(t, tv.String(), fmt.Sprintf("%d", 12345678))
	assert.DeepEqual(t, tv.Data, []uint8{0x4E, 0x61, 0xBC, 0x0, 0x0, 0x0, 0x0, 0x0})

	tv = NewInt64(testPositiveInt)
	assert.Equal(t, tv.String(), fmt.Sprintf("%d", testPositiveInt))
	assert.DeepEqual(t, tv.Data, []uint8{0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x7F})

	tv = NewInt64(testZeroInt)
	assert.Equal(t, tv.String(), fmt.Sprintf("%d", testZeroInt))
	assert.Equal(t, (*TypedValue)(tv).ValueToString(), "0")
	assert.DeepEqual(t, tv.Data, []uint8{0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0})
}
...
func Test_TypedValueUint(t *testing.T) {
	...
	assert.DeepEqual(t, tv.Data, []uint8{0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF})
...
func Test_TypedValueBool(t *testing.T) {
	...
	assert.Equal(t, len(tv.Data), 1)
	assert.Equal(t, tv.String(), "true")
	assert.DeepEqual(t, tv.Data, []uint8{0x01})
...
func Test_TypedValueDecimal64(t *testing.T) {
	...
	assert.Equal(t, (*TypedValue)(tv).ValueToString(), "0")
	assert.DeepEqual(t, tv.Data, []uint8{0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x00})
}
...
func Test_TypedValueFloat(t *testing.T) {
	...
	assert.DeepEqual(t, tv.Data, []uint8{0x0, 0x0, 0x0, 0xC0, 0x33, 0xF9, 0xEF, 0xC7})
	...
	assert.Equal(t, len(tv.TypeOpts), 0)
	assert.DeepEqual(t, tv.Data, []uint8{0x0, 0x0, 0x0, 0xC0, 0x33, 0xF9, 0xEF, 0x47})
}

func Test_TypedValueBytes(t *testing.T) {
	tv := NewBytes([]byte(testStringBytes))
	assert.Equal(t, len(tv.Data), 11)
...
func Test_JsonSerializationString(t *testing.T) {
	...
	assert.DeepEqual(t, unmarshalledTv.Data, []byte{0x54, 0x68, 0x69, 0x73, 0x20, 0x69, 0x73, 0x20, 0x61, 0x20, 0x74, 0x65, 0x73, 0x74})
...
func Test_JsonSerializationDecimal(t *testing.T) {
	...
	assert.DeepEqual(t, unmarshalledTv.Data, []byte{0xd0, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00})
...
func Test_JsonSerializationInt(t *testing.T) {
	...
	assert.Equal(t, len(unmarshalledTv.TypeOpts), 0)
	assert.DeepEqual(t, unmarshalledTv.Data, []byte{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80})
...
func Test_JsonSerializationUint(t *testing.T) {
	...
	assert.Equal(t, len(unmarshalledTv.TypeOpts), 0)
	assert.DeepEqual(t, unmarshalledTv.Data, []byte{0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00})
...
func Test_JsonSerializationBool(t *testing.T) {
	...
	assert.Equal(t, len(unmarshalledTv.TypeOpts), 0)
	assert.DeepEqual(t, unmarshalledTv.Data, []byte{0x1})
...
func Test_JsonSerializationLeafListInt(t *testing.T) {
	...
	assert.Equal(t, len(unmarshalledTv.TypeOpts), 0)
	assert.Equal(t, len(unmarshalledTv.Data), 24)
	assert.DeepEqual(t, unmarshalledTv.Data, []byte{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x7f})
...
func Test_JsonSerializationLeafListBytes(t *testing.T) {
	...
	assert.DeepEqual(t, unmarshalledTv.Data, []byte{0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x67, 0x68, 0x69, 0x6a, 0x6b})
